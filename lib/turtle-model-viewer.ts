...
export default class TurtleModelViewer extends HTMLElement {
  shadow: ShadowRoot
  ...
  private async build(options: { productSlug: string; shadow: ShadowRoot }) {
    const product = await this.getProduct({ productSlug: options.productSlug })
  ...
  static async setup(w: Window, apiToken: string) {
    await authenticateCustomer(apiToken)
...
declare global {
  interface HTMLElementTagnameMap {
    // 'turtle-model-viewer': ReturnType<typeof createTurtleModelViewer>
    'turtle-model-viewer': TurtleModelViewer
  }
}
